...
valgrind $BINDIR/sfsz -r 1024 $TESTDIR/data1 $TESTDIR/data1.cbz
...
# Convert long to little endian binary format
objcopy -I binary -O binary --reverse-bytes=8 $TESTDIR/tmp1 $TESTDIR/craft1
dd if=$TESTDIR/craft1 of=$TESTDIR/data1.cbz conv=notrunc seek=$((8224+1024)) oflag=seek_bytes
valgrind $BINDIR/sfsuz $TESTDIR/data1.cbz $TESTDIR/data2
...
# Convert long to little endian binary format
objcopy -I binary -O binary --reverse-bytes=8 $TESTDIR/tmp1 $TESTDIR/craft1
dd if=$TESTDIR/craft1 of=$TESTDIR/data1.cbz conv=notrunc seek=$((8216+1024)) oflag=seek_bytes
valgrind $BINDIR/sfsuz $TESTDIR/data1.cbz $TESTDIR/data2
...
# Convert long to little endian binary format
objcopy -I binary -O binary --reverse-bytes=8 $TESTDIR/tmp1 $TESTDIR/craft1
dd if=$TESTDIR/craft1 of=$TESTDIR/data1.cbz conv=notrunc seek=$((8232+1024)) oflag=seek_bytes
valgrind $BINDIR/sfsuz $TESTDIR/data1.cbz $TESTDIR/data2
...
# Convert long to little endian binary format
objcopy -I binary -O binary --reverse-bytes=8 $TESTDIR/tmp1 $TESTDIR/craft1
dd if=$TESTDIR/craft1 of=$TESTDIR/data1.cbz conv=notrunc seek=$((8256+1024)) oflag=seek_bytes
valgrind $BINDIR/sfsuz $TESTDIR/data1.cbz $TESTDIR/data2

reset_data1

echo "Craft 8-1: crafting fake random buffer byte size (random buffer switched off when actually not)"
# echo "0: 01" | xxd -r -g0 > $TESTDIR/random_on
printf "0: %.16x" 0 |  xxd -r -g0 > $TESTDIR/random_size_bytes_rev
...
echo "Craft 8-2: crafting fake random buffer byte size (random buffer bad size)"
printf "0: %.16x" 1025 |  xxd -r -g0 > $TESTDIR/random_size_bytes_rev
objcopy -I binary -O binary --reverse-bytes=8 $TESTDIR/random_size_bytes_rev $TESTDIR/random_size_bytes
...
# Random buffer flag
# /usr/bin/od -An -N 1 -t u1 -j 0

# First atomic block useful data size
# /usr/bin/od -An -N 8 -t u8 -j 1 data1.cbz
# 8192

# First atomic block array size
# /usr/bin/od -An -N 8 -t u8 -j $((8192+8+1)) data1.cbz
# 4 (2 non sparse block boundaries, relative offsets)
# [0, 4096, 4096, 4096]

# Total read
# /usr/bin/od -An -N 8 -t u8 -j $((8248+1)) data1.cbz
# 16384

# Total written
# /usr/bin/od -An -N 8 -t u8 -j 8257 data1.cbz
# 8280 (1 + 8 + 8192 + 8 + 4 * 8 + 8 = final footer delim -1L (ffff ffff ffff ffff, see sfsz.c) + 32)

# ratio, yet to be parsed correctly as a double
#/usr/bin/od -An -N 8 -f -j 8265 data1.cbz

# Number of atomic blocs
# /usr/bin/od -An -N 8 -t u8 -j 8273 data1.cbz
